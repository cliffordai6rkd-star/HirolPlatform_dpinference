...
from learning_factory.configs.types import FeatureType, PolicyFeature
from learning_factory.datasets.backward_compatibility import (
    FUTURE_MESSAGE,
    BackwardCompatibilityError,
    ForwardCompatibilityError,
)
from learning_factory.utils.constants import ACTION, OBS_ENV_STATE, OBS_STR
from learning_factory.utils.utils import SuppressProgressBars, is_valid_numpy_dtype_string
...
_HF_TO_TENSOR = transforms.ToTensor()
def item_to_torch(item: dict) -> dict:
    ...
    for key, val in item.items():
        if isinstance(val, PILImage.Image):
            item[key] = _HF_TO_TENSOR(val)  # -> float32, C×H×W, [0,1]
        elif isinstance(val, (np.ndarray | list)) and key not in ["task"]:
            item[key] = torch.as_tensor(val)
    return item
