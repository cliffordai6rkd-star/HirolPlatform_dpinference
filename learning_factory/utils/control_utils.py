...
from learning_factory.datasets.utils import DEFAULT_FEATURES
...
from learning_factory.robots import Robot
    ...
    def on_press(key):
        try:
            if key == keyboard.Key.right:
                ...
                print("Enter key pressed. Ready for next episode...")
                events["proceed_next_episode"] = True
            else:
                char = getattr(key, "char", None)
                if char is None:
                    return
                if char.lower() == "s":
                    print("'s' key pressed. Marking episode success...")
                    events["episode_success"] = True
                    events["episode_failure"] = False
                elif char.lower() == "f":
                    print("'f' key pressed. Marking episode failure...")
                    events["episode_failure"] = True
                    events["episode_success"] = False
