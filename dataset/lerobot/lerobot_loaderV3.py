from dataset.lerobot.readerV3 import ActionType, Action_Type_Mapping_Dict, ObservationType
...
class LerobotLoader(DataLoaderBase):
    ...
    def convert_dataset(self):
        ...
        for i, key in enumerate(self._image_keys):
            dataset_key = f"observation.images.{key}"
            feature_dicts[dataset_key] = {
                "dtype": "video",
        ...
        feature_dicts[obs_state_key] = {
            "dtype": "float64",
            "shape": (self._obs_states_dim,),
            "names": ["state"],
        }
        feature_dicts["action"] = {"dtype": "float64",
            ...
            for cur_episode_dir in tqdm(dirs, desc=f"processing episodes", unit="episode"):
                ...
                for num_step, step in tqdm(enumerate(episode_data), desc=f"processing steps", unit="step"):
                    ...
                    for image_key in self._image_keys:
                        dataset_key = f"observation.images.{image_key}"
                        step_key = "colors" if "color" in image_key else "depths"
                        frame_feature[dataset_key] = step[step_key][image_key]



                    # obs states
                    frame_feature[obs_state_key] = np.array([])
                    obs_states = step["observations"]
                    for key, obs_state in obs_states.items():
                        frame_feature[obs_state_key] = np.hstack((frame_feature[obs_state_key], obs_state))
                    # print(f'state: {frame_feature[obs_state_key]}')  # 注释掉频繁的调试输出
                    if len(frame_feature[obs_state_key]) != self._obs_states_dim:
                        log.warn(f'{task_dir} {episode_dir} has wrong state dim: {len(frame_feature[obs_state_key])} in {num_step}th step')
                        ...
                        continue
                    # actions
                    frame_feature["action"] = np.array([])
                    for key, value in step["actions"].items():
                        frame_feature["action"] = np.hstack((frame_feature["action"], value))
                    # print(f'action: {frame_feature["action"]}')  # 注释掉频繁的调试输出
...
if __name__ == '__main__':
    import yaml
    task_list = [{'name': "pick_and_place_cube_chaos", 'task_dir': '/home/hanyu/code/HIROLRobotPlatform/dataset/data/1016_pick_and_place_fr3_3dmouse_100_chaos', 
                  'cfg_file': '/home/hanyu/code/HIROLRobotPlatform/dataset/lerobot/config/pick_and_place_config'},
                ]
    ...
    lerobot_dataset = LerobotLoader(config, task_dir, action_type=action_type, observation_type=obs_type)

    repo_root = os.path.join(cur_path, config.get("root_path", "../assets/data"))
    dataset_root = os.path.join(repo_root, config["repo_name"])
    info_path = os.path.join(dataset_root, "meta", "info.json")

    if not os.path.exists(info_path):
        writer_dataset = lerobot_dataset.convert_dataset()
        # convert_dataset already finalizes internally, but call defensively in case of early exits
        writer_dataset.finalize()
    else:
        log.info(f"Dataset already exists at {dataset_root}. Skipping conversion.")

    dataset = LeRobotDataset(repo_id=config["repo_name"], root=dataset_root)
