...
class RerunEpisodeReader:
        ...
        for i, item_data in enumerate(json_file['data']):
            ...
            if self.action_type == ActionType.JOINT_POSITION:
                ...
                if self._action_ori_type == "euler":
                    ...
                    raise ValueError(f'The action orientation type {self._action_ori_type} is not supported for reading episode data')
            elif self.action_type == ActionType.COMMAND_JOINT_POSITION:
                command_states = self._get_action_slice(item_data, "joint", "position")
                if command_states is None or len(command_states) == 0:
                    raise ValueError(f'Do not get command joint position actions from {self.task_dir} {episode_dir}')
                cur_actions = self._get_absolute_action(command_states, command_states, "position")
            elif self.action_type == ActionType.COMMAND_JOINT_POSITION_DELTA:
                command_states = self._get_action_slice(item_data, "joint", "position")
                next_command_states = self._get_action_slice(json_data[action_state_id], "joint", "position")
                if command_states is None or next_command_states is None or len(command_states) == 0:
                    raise ValueError(f'Do not get command joint position actions from {self.task_dir} {episode_dir}')
                cur_actions = self._get_delta_action(command_states, next_command_states, "position")
            elif self.action_type == ActionType.COMMAND_END_EFFECTOR_POSE:
                command_states = self._get_action_slice(item_data, "ee", "pose")
                if command_states is None or len(command_states) == 0:
                    raise ValueError(f'Do not get command end effector pose actions from {self.task_dir} {episode_dir}')
                cur_actions = self._get_absolute_action(command_states, command_states, "pose")
                if self._action_ori_type == "euler":
                    modified_action = {}
                    for key, action in cur_actions.items():
                        modified_action[key] = np.zeros(6)
                    ...
                    raise ValueError(f'The action orientation type {self._action_ori_type} is not supported for reading episode data')
            elif self.action_type == ActionType.COMMAND_END_EFFECTOR_POSE_DELTA:
                command_states = self._get_action_slice(item_data, "ee", "pose")
                next_command_states = self._get_action_slice(json_data[action_state_id], "ee", "pose")
                if command_states is None or next_command_states is None:
                    raise ValueError(f'Do not get command end effector pose actions from {self.task_dir} {episode_dir}')
                for key, pose in command_states.items():
                    if key not in next_command_states:
                        log.warn(f'command action key {key} not found in next step for {self.task_dir} {episode_dir}')
                        continue
                    cur_actions[key] = np.zeros(7)
                    next_pose = np.array(next_command_states[key]["pose"])
                    next_pose = self.apply_rotation_offset(next_pose, key)
    ...
    def _extract_action_section(self, actions, target_key):
        """Recursively search for a specific command section inside the actions dict."""
        if not isinstance(actions, dict):
            return None
        if target_key in actions:
            return actions[target_key]
        for value in actions.values():
            if isinstance(value, dict):
                result = self._extract_action_section(value, target_key)
                if result is not None:
                    return result
        return None

    def _get_action_slice(self, item_data, section_key, attribute_name):
        """Extract a specific action portion directly from actions dict."""
        actions = item_data.get("actions", {})
        if not isinstance(actions, dict):
            return None
        result = {}
        for key, action_dict in actions.items():
            if not isinstance(action_dict, dict):
                continue
            section = action_dict.get(section_key)
            if section is None or attribute_name not in section:
                continue
            result[key] = {attribute_name: section[attribute_name]}
